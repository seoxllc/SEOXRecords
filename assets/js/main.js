...
(function () {
  ...
  window.seoxGetFirebase = function seoxGetFirebase() {
    ...
    _firebasePromise = (async () => {
      try {
        ...
        // Firestore's default connection (WebChannel/gRPC-style streaming) gets
        // silently blocked on some networks, VPNs, and browser extensions —
        // requests just hang instead of erroring. Forcing long-polling uses a
        // plain HTTP transport instead, which is far more compatible.
  ...
  /* ---------------------------------------------------------------------
     ...
     Both network calls below are capped with a timeout — if Firestore or
     the Formspree endpoint hang (bad rules, unauthorized domain, network/
     ad-blocker issues, slow server, etc.) the form still completes instead
     of getting stuck on "Sending…" forever.
     --------------------------------------------------------------------- */
  ...
  document.querySelectorAll("form[data-form]").forEach((form) => {
    form.addEventListener("submit", async (e) => {
      ...
      // Add ?debug=1 to the page URL to see exactly what Firebase/Formspree
      // did (or didn't do) printed directly on the page — no dev tools needed.
      ...
      if (hasRealEndpoint) {
        try {
          ...
          if (result && result.__timedOut) {
            console.warn('Form endpoint (action="' + action + '") took too long — continuing without waiting for it.');
            debugLines.push("Email endpoint: timed out after 6s");
          } else {
            debugLines.push("Email endpoint: responded (status " + result.status + ")");
          }
        } catch (err) {
          debugLines.push("Email endpoint: failed — " + err.message);
        }
      } else if (debugMode) {
        debugLines.push("Email endpoint: skipped (no action= URL set on the form)");
      }
      ...
      if (form.dataset.store === "submissions") {
        ...
        try {
          const fb = await withTimeout(window.seoxGetFirebase(), 6000);
          if (fb && fb.__timedOut) {
            console.warn("Firebase took too long to respond — check your project config, Firestore rules and authorized domains. Continuing without it.");
            debugLines.push("Firebase: timed out loading the SDK/connecting (6s)");
          } else if (fb) {
            debugLines.push("Firebase: SDK loaded OK, project = " + (window.SEOX_FIREBASE_CONFIG && window.SEOX_FIREBASE_CONFIG.projectId));
            const { collection, addDoc, serverTimestamp } = fb.firestore;
            const writeResult = await withTimeout(
              addDoc(collection(fb.db, "submissions"), { fields, receivedAt: serverTimestamp() }),
              6000
            );
            if (writeResult && writeResult.__timedOut) {
              debugLines.push("Firestore write: timed out after 6s (likely rules or authorized-domain issue)");
            } else {
              debugLines.push("Firestore write: SUCCESS — doc id " + writeResult.id);
            }
          } else {
            debugLines.push("Firebase: not configured (firebase-config.js still has placeholder values) — skipped");
          }
        } catch (err) {
          console.warn("Firestore sync failed — submission still saved locally.", err);
          debugLines.push("Firestore write: FAILED — " + (err && err.code ? err.code + ": " : "") + (err && err.message ? err.message : err));
        }
        ...
        try {
          ...
          debugLines.push("Local copy: saved to this browser's storage");
        } catch (err) {
          console.warn("Could not save submission locally:", err);
          debugLines.push("Local copy: FAILED — " + err.message);
        }
      } else if (debugMode) {
        debugLines.push('Firebase/local save: skipped (form is missing data-store="submissions")');
      }
      ...
      if (!debugMode) form.reset();
      const dynamicArtists = form.querySelector("[data-artist-fields]");
      if (dynamicArtists && !debugMode) dynamicArtists.dispatchEvent(new Event("seox:reset"));
    });
  });
